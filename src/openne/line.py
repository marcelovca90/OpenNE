...
class _LINE(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=1000, negative_ratio=5, order=3):
        ...
        initializer = tf.compat.v1.glorot_normal_initializer(seed=cur_seed)
    ...
    def build_graph(self):
        ...
        self.h = tf.compat.v1.placeholder(tf.int32, shape=[None])
        self.t = tf.compat.v1.placeholder(tf.int32, shape=[None])
        self.sign = tf.compat.v1.placeholder(tf.float32, shape=[None])

        cur_seed = random.getrandbits(32)
        self.embeddings = tf.compat.v1.get_variable(name="embeddings"+str(self.order), shape=[
                                          int(self.node_size), int(self.rep_size)], initializer=tf.compat.v1.glorot_normal_initializer(seed=cur_seed))
        ...
        self.second_loss = -tf.reduce_mean(tf.compat.v1.log_sigmoid(
            self.sign*tf.reduce_sum(tf.multiply(self.h_e, self.t_e_context), axis=0)))
        self.first_loss = -tf.reduce_mean(tf.compat.v1.log_sigmoid(
            self.sign*tf.reduce_sum(tf.multiply(self.h_e, self.t_e), axis=0)))
        if self.order == 1:
